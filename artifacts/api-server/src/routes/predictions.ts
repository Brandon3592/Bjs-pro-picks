...
function kellyStake(modelProb: number, odds: number): number {
  ...
  return Math.max(0, Math.min(kelly * 0.25, 0.1)); // quarter kelly, capped at 10%
}
...
const mockValueBets = [
  ...
  },
  {
    ...
    bookmaker: "PointsBet",
    odds: +105,
    impliedProb: americanOddsToImplied(105),
    modelProb: 0.51,
    edge: 3.7,
    kellyStake: kellyStake(0.51, 105),
    status: "upcoming",
  },
  {
    id: "vb-6",
    ...
    bookmaker: "DraftKings",
    odds: -118,
    impliedProb: americanOddsToImplied(-118),
    modelProb: 0.598,
    edge: 3.9,
    kellyStake: kellyStake(0.598, -118),
    status: "live",
  },
  {
    id: "vb-7",
  ...
  },
  {
    ...
    bookmaker: "BetMGM",
    odds: -107,
    impliedProb: americanOddsToImplied(-107),
    modelProb: 0.537,
    edge: 3.3,
    kellyStake: kellyStake(0.537, -107),
    status: "upcoming",
  },
];
...
router.get("/predictions", (req, res) => {
  ...
  const predictions = mockValueBets.map((vb) => ({
    gameId: vb.gameId,
    sport: vb.sport,
    homeWinProb: vb.modelProb,
    awayWinProb: parseFloat((1 - vb.modelProb).toFixed(3)),
    drawProb: null,
    edgeHome: vb.edge,
    edgeAway: parseFloat((-vb.edge / 2).toFixed(2)),
